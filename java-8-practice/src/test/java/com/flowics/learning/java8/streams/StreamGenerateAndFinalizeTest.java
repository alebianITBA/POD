...
public class StreamGenerateAndFinalizeTest {
    ...
    public final void of_element_find_first() {
        ...
        assertEquals("Should be getting the first element", new Integer(34), numbers.findFirst().get());
    }
    ...
    public final void builder_find_any() {
        ...
        assertTrue("use find any to match", Arrays.asList(1, 2, 3, 4, 5).contains(numbers.findAny().get()));
    }

    @Test
    public final void of_list_any_match() {
        final Stream<Integer> numbers = Stream.of(32, 4, 12, 3, 2, 1);
        assertEquals("Should be true if any match 2", true, numbers.anyMatch(p -> p == 2));
    }

    @Test
    public final void iterate_all_match() {
        final Stream<Integer> numbers = Stream.iterate(1, x -> x + 2); // infinite stream
        // as all match will break with the first non complient it will not be an infinite loop.
        assertEquals("Use all match to uneven", false, numbers.allMatch(p -> p % 2 == 0));
    }

    @Test
    public final void empty_none_match() {
        final Stream<Integer> numbers = Stream.empty();
        assertEquals("None match should match to 1", true, numbers.noneMatch(p -> p == 1));
    }

    @Test
    public final void concat_min_max() {
        Stream<Integer> numbers = Stream.concat(Stream.of(32), Stream.of(31, 33));
        assertEquals("Should return the min", new Integer(31), numbers.min(Comparator.naturalOrder()).get()); // check Comparator#naturalOrder
        numbers = Stream.concat(Stream.of(32), Stream.of(31, 33));
        assertEquals("Should return the max", new Integer(33), numbers.max(Comparator.naturalOrder()).get()); // check Comparator#naturalOrder
    }
    ...
    public final void double_stream_sum() {
        ...
        assertEquals("should get the  sum of the values", 6.9, numbers.reduce(0, Double::sum), 0.001);
    }
}
